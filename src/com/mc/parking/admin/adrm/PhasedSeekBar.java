...
public class PhasedSeekBar extends View {
    ...
    protected void configure() {
        ...
        for (int i = 0, j = 1; i < count; i++, j++) {
            mAnchors[i][0] = mModeIsHorizontal ? widthBase * j - widthHalf + rect.left+20 : mPivotX;
            mAnchors[i][1] = !mModeIsHorizontal ? heightBase * j - heightHalf + rect.top : mPivotY;
        }
        //直接设置两个图片的位置
        mAnchors[0][0]=(int) mBackgroundPaddingRect.left;
        mAnchors[1][0]=(int) (getWidth() - mBackgroundPaddingRect.right);
        
        mylenth= Math.abs(mAnchors[1][0]-mAnchors[0][0]); 
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!mUpdateFromPosition) {
            ...
            distance=Math.abs(mModeIsHorizontal ? mAnchors[0][0] - mCurrentX : mAnchors[0][1] - mCurrentY);
            distance=Math.abs(mAnchors[0][0] - mCurrentX);
            if(distance>(mylenth-20))
            {
            	setCurrentItem(1);
            	  stateListDrawable = mAdapter.getItem(1);
            }else if(distance<20)
            {
            	setCurrentItem(0);
            	  stateListDrawable = mAdapter.getItem(0);
            }
            
//            for (int i = 0; i < count; i++) {
//               distance = Math.abs(mModeIsHorizontal ? mAnchors[i][0] - mCurrentX : mAnchors[i][1] - mCurrentY);
//            	
////                if(Math.abs(distance-mylenth)<20)
////                {
////                	if(mCurrentItem!=i)
////                		minIndex = i;
////                	
...
////                }
//              Log.e("parkinfotext","bbb"+distance);
//            }
