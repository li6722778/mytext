...
public class LoginDialogFragment extends DialogFragment
{
	...
	public LoginDialogFragment(Activity activity,TParkInfo_LocEntity tParkInfo_LocEntity ){
	...
	
	{
		...
		//Toast.makeText(getActivity(), ""+fManager, Toast.LENGTH_LONG).show();
		...
		builder.setView(view)
				...
				.setPositiveButton("登录",
						new DialogInterface.OnClickListener()
						{
							...
							{
								if (!UIUtils.validationNotEmpty(activity,"手机号码", mUsername)) {
									return;
								}
								if (!UIUtils.validationNotEmpty(activity,"密码", mPassword)) {
									return;
								}
							//	new LoginTask(activity,loginhandler,mUsername.getText().toString(),mPassword.getText().toString()).execute();
								HttpRequestAni<TuserInfo> httpRequestAni = new HttpRequestAni<TuserInfo>(
										activity, "/a/login/" + mUsername.getText().toString()+"?currentVersion="+Constants.VERSION+"&os=android",
										new TypeToken<TuserInfo>() {}.getType()) {
									@Override
									public void callback(TuserInfo arg0) {
										...
										if (SessionUtils.loginUser.userType >= Constants.USER_TYPE_PADMIN
											...
											HttpRequestAni<List<TParkInfoEntity>> httpRequestAni = new HttpRequestAni<List<TParkInfoEntity>>(
													activity, "/a/user/adm/"+ arg0.userid,
													...
													if (arg0 != null && arg0.size() > 0) {
														if (arg0.size() == 1) {
															...
															if (parkList != null&& parkList.size() > 0) {
																...
																dialog.show(fManager,"parklistdialog");
																// 弹出完毕
															}
														}
									...
									public Map<String, String> getHeaders() {
										...
										String creds = String.format("%s:%s", mUsername.getText().toString(),
						...
						}).setNegativeButton("取消", null).setNeutralButton("注册",new DialogInterface.OnClickListener()
						{
							@Override
							public void onClick(DialogInterface dialog, int id)
							{
								...
								intent.putExtra("userphone", mUsername.getText().toString());
        ...
        public void handleMessage(Message msg) {  
            ...
            if(activity!=null){
            	...
            	if(activity instanceof MainActivity ){
            		...
            		((MainActivity) activity).mapFragment.gotoPage();
