...
public class AdminServiceActivity extends FragmentActivity {

	ServiceOrderFragment orderFrag;
	private BaseViewPagerIndicator mIndicator = null;
	private ViewPager viewPager;
	private String[] columns = new String[] { "未完成服务", "已完成服务" };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 保存当前实例
		PackingApplication.getInstance().setCurrentActivity(AdminServiceActivity.this);
	...
	public void reloadOrderList() {
		if (tabFragments != null) {
			for (Fragment orderFragment : tabFragments) {
				if (orderFragment instanceof ServiceOrderFragment) {
					((ServiceOrderFragment) orderFragment).reload();
					break;
				}
			}
		}
	...
	protected void onResume() {
		...
		if (SessionUtils.loginUser == null) {
			TuserInfo userInfo = (TuserInfo) SessionUtils.readUserinfo(AdminServiceActivity.this, "userinfo");
				...
				if (resultday <= Constants.loginperiod) {
					...
					if (SessionUtils.loginUser.userType >= Constants.USER_TYPE_PADMIN
						...
						TParkInfoEntity tParkInfoEntity = (TParkInfoEntity) SessionUtils
								.readchoiceparkInfoAdm(AdminServiceActivity.this, "parkinfo");
	...
	private void initview() {
		tabFragments = new android.support.v4.app.Fragment[] { new ServiceOrderFragment(), new ServiceHisFragment() };
	...
	public void refreshfragment() {
		...
		// CaptureFragment chooserFrag = (CaptureFragment)
		// fm.findFragmentByTag("chooser");
		orderFrag = (ServiceOrderFragment) fm.findFragmentByTag("order");
		// 刷新
		// if(orderFrag==null)
		...
		AlertDialog.Builder alertDialog = new AlertDialog.Builder(AdminServiceActivity.this);
