...
public class YuyueActivity extends ActionBaseActivity {
		...
		youhuilLayout.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (priceEntity.payActualPrice <= 0.001 && COUPON_CHECK_FLAG == 0) {
					Toast.makeText(YuyueActivity.this, "已不需要支付", Toast.LENGTH_LONG).show();
					return;
				}

				Intent intent = new Intent(YuyueActivity.this, CouponDetailActivity.class);
	...
	protected void onActivityResult(int requestcode, int resultcode, Intent data) {
...

		{
			...
			if (couponBean != null && couponBean.counponId != 0) {
				...
				Toast.makeText(YuyueActivity.this, "停车券使用过程中出错，请重试", Toast.LENGTH_LONG).show();
	...
	public void showphoto(View v) {
		...
		case R.id.packdetail_img_frame:
			Intent intent = new Intent(YuyueActivity.this, UploadPhotoActivity.class);
	...
	private void getCalculatedPrice(final OrderEntity orderInfo) {
		...
		if (parkInfo == null) {
			Toast.makeText(YuyueActivity.this, "加载出错，请重新选", Toast.LENGTH_LONG).show();
			return;
	...
	private void startToPay() {
		...
		HttpRequest<ComResponse<ChebolePayOptions>> httpRequestAni = new HttpRequest<ComResponse<ChebolePayOptions>>(
				...
				"/a/pay/innew/" + parkInfo.parkId + "?c=" + parastring + "&lt=" + mCurrentLantitude + "&ln="
						+ mCurrentLongitude + "&payway=" + selectedPayWay + "&clientId=" + clientid + "&s=1",
	...
	public void pay(View v) {
		...
		if (!message.trim().equals("")) {
			AlertDialog.Builder alertDialog = new AlertDialog.Builder(YuyueActivity.this);
	...
	private void paychangeStatus(int paystatus, String message) {
		...
		if (paystatus == Constants.PAYMENT_STATUS_FINISH || paystatus == Constants.PAYMENT_STATUS_PENDING) {
			...
			alertDialog.setNegativeButton("查看订单", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int id) {
					Intent intent = new Intent(YuyueActivity.this, OrderDetailActivity.class);
					Bundle buidle = new Bundle();
					buidle.putSerializable("orderinfo", priceEntity.order);
