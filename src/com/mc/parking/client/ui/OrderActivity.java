...
import com.mc.parking.client.ui.fragment.LoginDialogFragment;
import com.mc.parking.client.ui.fragment.MainFragment;
...
public class OrderActivity extends  FragmentActivity implements IXListViewListener {
	...
	Fragment fragment;
	TextView titleView,servicetitleView;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE, R.layout.userorder_topbar);
		 titleView = (TextView) findViewById(R.id.topbar_title_order);
		titleView.setText(R.string.rightmenu_parkhis_title);
		servicetitleView=(TextView) findViewById(R.id.topbar_title_serviceorder);
		 manager =getSupportFragmentManager();
			myFragment = (UserOrderFragment) manager.findFragmentById(R.id.userorder);
		...
		titleView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				ft = manager.beginTransaction();
				 Fragment ff=manager.findFragmentById(R.id.userorder);
				 if(ff instanceof UserOrderFragment)
				 {
				
				 }else
				 {
					 titleView.setBackgroundResource(R.drawable.icontabselectedleft);
					 servicetitleView.setBackgroundResource(R.drawable.icontabnoselectright);
					 ft.replace(R.id.userorder, new UserOrderFragment());
				 }
				ft.commit();
			}
		});
		
		servicetitleView.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				fragment=new UserBannerOrderFragment();
				ft = manager.beginTransaction();
				 Fragment ff=manager.findFragmentById(R.id.userorder);
				if(ff instanceof UserOrderFragment)
				 {
					titleView.setBackgroundResource(R.drawable.icontabnoselectleft);
					servicetitleView.setBackgroundResource(R.drawable.icontabselectedright);
					ft.replace(R.id.userorder,fragment );
				
				 }
				ft.commit();
			}
		});
	/*	baseviewpage_tv1 = (TextView) findViewById(R.id.baseviewpage_tv1);
		baseviewpage_tv2 = (TextView) findViewById(R.id.baseviewpage_tv2);
		baseviewpage_tv3 = (TextView) findViewById(R.id.baseviewpage_tv3);

		baseviewpage_numb1 = (TextView) findViewById(R.id.baseviewpage_numb1);
		baseviewpage_numb2 = (TextView) findViewById(R.id.baseviewpage_numb2);
		baseviewpage_numb3 = (TextView) findViewById(R.id.baseviewpage_numb3);

		indicator_first = (RelativeLayout) findViewById(R.id.indicator_first);
		indicator_second = (RelativeLayout) findViewById(R.id.indicator_second);
		indicator_thrid = (RelativeLayout) findViewById(R.id.indicator_thrid);
		...
		Integer selectTab = getIntent().getIntExtra("selectedTab", 0);

		hasCheckedView2 = false;
		hasCheckedView3 = false;
		initViewPager(selectTab);
		if (selectTab == 0) {
			initlistview();
	...
	private void initlistview() {
		...
		listView1.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				 ...
				 * Toast.makeText(OrderActivity.this, "当前订单已经过期,请激活使用",
				 * Toast.LENGTH_LONG) .show(); return; }
				 * 
				 * if(record.getOrderStatus()==Constants.ORDER_TYPE_EXCPTION){
				 * Toast.makeText(OrderActivity.this, "当前订单异常,请联系管理员",
				 * Toast.LENGTH_LONG) .show(); return; }
				 * 
					...
					if (orderEntity.getOrderStatus() == Constants.ORDER_TYPE_START) {
						...
						if (infoEntity != null) {
							...
							if (!foundHasPayment) {
								...
								Toast.makeText(OrderActivity.this, "当前订单已经付费,您不能取消", Toast.LENGTH_LONG).show();
	...
	private void initViewPager(int selectTab) {
	...
	*//**
	...
	public abstract class PullToRefreshListViewAdapter extends android.widget.BaseAdapter {
		...
		*//**
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			holder.orderDate.setText(UIUtils.formatDate(OrderActivity.this, record.getOrderDate().getTime()));
	...
	public void getfirstdata() {
		HttpRequestAni<CommFindEntity<OrderEntity>> httpRequestAni = new HttpRequestAni<CommFindEntity<OrderEntity>>(
				OrderActivity.this, "/a/order/mepay/2", new TypeToken<CommFindEntity<OrderEntity>>() {
				}.getType()) {
	...
	private void getfirstdataForView2() {
		HttpRequestAni<CommFindEntity<OrderEntity>> httpRequestAni = new HttpRequestAni<CommFindEntity<OrderEntity>>(
				OrderActivity.this, "/a/orderhis/me", new TypeToken<CommFindEntity<OrderEntity>>() {
				}.getType()) {
	...
	public void onloadMoreForView2() {
		...
		if (currentcountView2 > totalcountView2 - 1) {
			...
			Toast.makeText(OrderActivity.this, "没有更多数据", Toast.LENGTH_SHORT).show();
			listView2.setPullLoadEnable(false);
			return;
		...
		HttpRequest<CommFindEntity<OrderEntity>> httpRequestAni = new HttpRequest<CommFindEntity<OrderEntity>>(
			...
			@Override
			public void onFailed(String message) {
				Toast.makeText(OrderActivity.this, "加载列表失败", Toast.LENGTH_SHORT).show();
			}
	...
	*//**
		...
		HttpRequest<ComResponse<OrderEntity>> httpRequestAni = new HttpRequest<ComResponse<OrderEntity>>(
			...
			public void onSuccess(ComResponse<OrderEntity> arg0) {
				if (arg0.getResponseStatus() == ComResponse.STATUS_OK) {
					Toast.makeText(OrderActivity.this, "取消订单成功", Toast.LENGTH_SHORT).show();
					getfirstdataforUnPay();
					adapter3.notifyDataSetChanged();
				} else {
					Toast.makeText(OrderActivity.this, "取消订单失败", Toast.LENGTH_SHORT).show();
				}
			...
			@Override
			public void onFailed(String message) {
				Toast.makeText(OrderActivity.this, "删除订单失败", Toast.LENGTH_SHORT).show();
				Notice.errorotice();
			}
		...
		HttpRequest<ComResponse<OrderEntity>> httpRequestAni = new HttpRequest<ComResponse<OrderEntity>>(Method.POST,
			...
			public void onSuccess(ComResponse<OrderEntity> arg0) {
				if (arg0.getResponseStatus() == ComResponse.STATUS_OK) {
					...
					Toast.makeText(OrderActivity.this, "[异常]" + arg0.getErrorMessage(), Toast.LENGTH_SHORT).show();
					Notice.errorotice();
				}
			...
			@Override
			public void onFailed(String message) {
				Toast.makeText(OrderActivity.this, "[异常]", Toast.LENGTH_SHORT).show();
				Notice.errorotice();
			}
	...
	void getorderEntryByID(long id) {
		HttpRequestAni<OrderEntity> httpRequestAni = new HttpRequestAni<OrderEntity>(OrderActivity.this,
			...
			public void callback(OrderEntity arg0) {
				if (arg0 == null) {
					...
					if (record.getOrderStatus() == Constants.ORDER_TYPE_OVERDUE) {
						Toast.makeText(OrderActivity.this, "当前订单已经过期,请激活使用", Toast.LENGTH_LONG).show();
						getfirstdata();
						return;
					}

					if (record.getOrderStatus() == Constants.ORDER_TYPE_EXCPTION) {
						Toast.makeText(OrderActivity.this, "当前订单异常,请联系管理员", Toast.LENGTH_LONG).show();
						return;
					}
